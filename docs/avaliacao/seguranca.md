...
| SEG-06 | Para tarefas em produção, ou quando os riscos de segurança pertinentes à tarefa e ao seu Contexto de Uso justificarem, existem mecanismos proporcionais para detectar, registrar e responder a eventos ou incidentes de segurança relevantes?  | registros de monitoramento, procedimentos de resposta, registros de incidentes, evidências de ações corretivas | Análise de evidências operacionais; verificação documental; revisão técnica |
